...
/**
 * Allowed alias name pattern: the intersection of API Gateway stage name rules
 * (alphanumerics plus `-` and `_`, max 128 chars) and Lambda alias rules
 * (not `$LATEST`, not purely numeric).
 */
const VALID_ALIAS_NAME = /^(?!\$LATEST$)(?![0-9]+$)[a-zA-Z0-9_-]{1,128}$/;
...
class ServerlessLambdaAliasPlugin {
		...
		if (!VALID_ALIAS_NAME.test(this.config.alias)) {
			throw new this.serverless.classes.Error(
				`Invalid alias name '${this.config.alias}'. Alias names must match ${VALID_ALIAS_NAME} ` +
					'(alphanumerics, dashes, underscores; up to 128 characters; cannot be "$LATEST" or purely numeric). ' +
	 ...
	 *
	 * If the stack does not exist yet or the outputs are missing, this is a
	 * no-op and downstream code keeps its existing "no API ID -> skip" behavior.
	...
	async discoverApiIdsFromStack() {
		...
		try {
			...
			// ValidationError fires when the stack does not exist yet
	...
	async deployApiGateway() {
		try {
			...
			// Step 1: create a deployment snapshot anchored on the target alias stage.
			// Passing stageName binds the snapshot to that stage, creating the stage
			...
			// variable the integration URI references at runtime. The `replace`
			// patch op creates the variable if absent.
			...
			// variable set to its own stage name.
			if (TARGET_STAGE !== FRAMEWORK_STAGE) {
