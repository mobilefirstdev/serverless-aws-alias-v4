...
The integration URI templating (`:${stageVariables.alias}`) and stage-variable based alias routing already exist in v0.5.0 — they just only ever apply to one stage. This change extends them to additional stages.
...
- Integration URI template unchanged in behavior: still stage-variable based routing (`:${stageVariables.alias}`).
- Stage variable compatibility: plugin writes both `alias` and `SERVERLESS_ALIAS`.
...
- New constants `STAGE_VARIABLE_ALIAS = 'alias'` and `LEGACY_STAGE_VARIABLE_ALIAS = 'SERVERLESS_ALIAS'` (formalizes compatibility convention)
